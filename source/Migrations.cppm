...
template<class StateType, typename Callback>
...
class Migrations {
	class EasyMigrationAdder {
	public:
		EasyMigrationAdder(Migrations* migration_owner) : migration_owner(migration_owner) {}

		template<class... Args>
		EasyMigrationAdder& operator()(Args... args) {
			migration_owner->add(std::move(args)...);
			return *this;
		}
	private:
		Migrations* migration_owner;
	};
public:
	void add(const std::string version_string, const std::string sql_statement) {
		this->addMigration(std::make_unique<SQLOnlyMigration>(version_string, sql_statement));
	}
	template<class StateType, typename Callback>
	void add(const std::string version_string, StateType* state, Callback&& func) {
		this->addMigration(std::make_unique<SmartMigration<StateType, Callback>>(version_string, state, std::move(func)));
	}
	void addMigration(std::unique_ptr<Migration> migration) {
		this->migrations.push_back(std::move(migration));
	}
	EasyMigrationAdder addMigrations() { return EasyMigrationAdder(this); }

	// Populates database with entries in database_template.sql, and sets the version
	static void firstTimeSetup(FuzeDBI::Connection* fuze_dbi, const std::filesystem::path& template_path, const std::filesystem::path& absolute_sqlite_path, const std::string& current_version) {
	...
	void makeNewMigrations(FuzeDBI::Connection* fuze_dbi, const std::string& database_version_string, const std::string& server_version) {
		...
		for (const std::unique_ptr<Migration>& migration : migrations) {
			if (migration->version_string > server_version) {
				throw std::runtime_error(std::format("There is an error in configuration for migrations; migration has newer version {} than server_version {}", migration->version_string, server_version));
			}
